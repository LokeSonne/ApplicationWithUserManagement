    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user != null)
            {
                ...
                if (user.Activated == ActivationStatus.Pending) return View("ErrorPendingActivation");
                if (user.Activated == ActivationStatus.Rejected) return View("ErrorActivationRejected");
            }
        ...
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                var callbackUrl = Url.Action("ViewUserActivationRequest", "UserManagement", new { userId = userId }, protocol: Request.Url.Scheme);

                //Notify admin
                var admin = GetActivationNotificationRecipient();
                await UserManager.SendEmailAsync(admin.Id, "Forespørgsel om brugeraktivering", "En ny bruger ønsker adgang. Bekræft eller afkræft venligst forespørgslen <a href=\"" + callbackUrl + "\">her</a>");
            }
            return View(result.Succeeded ? "EmailConfirmed" : "Error");
        }

        private ApplicationUser GetActivationNotificationRecipient()
        {
            var userRoles = RoleAccess.GetAllUsersWithRole(RoleBasedEvents.ActivationRecipientRole);
